...
**(A)** : CNAME은 Zone Apex(루트 도메인, 예: example.com)에서 사용이 불가능하다. 서브도메인(예: www.example.com)에서만 사용 가능하다.

**(B) 정답** : Alias Record는 A/AAAA 타입으로 Zone Apex에서도 사용 가능하며 ALB는 Alias 대상으로 지원된다. DNS 쿼리 비용도 무료이다.
...
**(C) 정답** : Weighted 라우팅은 각 레코드에 상대적 가중치를 할당하여 트래픽 비율을 정밀하게 제어한다. 새 버전에 가중치 10, 기존 버전에 가중치 90을 설정하면 10%/90% 트래픽 분배가 가능하다. 카나리아 배포나 A/B 테스트에 적합하다.
...
**(A)** : Geolocation은 사용자의 지리적 위치(국가, 대륙)를 기반으로 라우팅하며 성능이 아닌 위치 기반이다. 독일 사용자가 미국 리전에서 더 낮은 지연을 경험해도 항상 유럽으로 라우팅될 수 있어 성능 최적화와 다르다.

**(B)** : Geoproximity는 지리적 근접성과 bias 값으로 영역 크기를 조절하는 기능이다. 실제 네트워크 성능(지연 시간)을 측정하지 않는다.

**(C) 정답** : Latency-based 라우팅은 사용자와 AWS 리전 간 네트워크 지연 시간을 측정하여 가장 낮은 지연 시간을 제공하는 리전으로 트래픽을 라우팅한다. "최상의 성능" 요구사항에 가장 정확히 부합한다.
...
**(B) 정답** : 올바른 방법은 CloudWatch Metric으로 DB를 모니터링하고, 이상 시 CloudWatch Alarm을 트리거한 후, Route 53 헬스 체크가 해당 CloudWatch Alarm 상태를 모니터링하도록 구성하는 것이다.
...
**(B) 정답** : Domain Registrar와 DNS Service는 별개의 개념이다. Route 53에서 Hosted Zone을 생성한 후, GoDaddy의 NS 레코드를 Route 53 Name Server 주소로 업데이트하면 DNS 관리를 Route 53에서 할 수 있다.
...
**(C) 정답** : Geolocation 라우팅은 사용자의 지리적 위치(대륙, 국가, 미국 주)를 기반으로 트래픽을 라우팅한다. 프랑스 → 프랑스 서버, 독일 → 독일 서버, 나머지 → Default 레코드로 정확히 매핑할 수 있다.
...
**(C) 정답** : EC2 인스턴스의 DNS 이름은 Alias Record 대상으로 설정할 수 없다. EC2에 트래픽을 보내려면 A 레코드에 퍼블릭 IP를 직접 지정하거나 ALB를 Alias 대상으로 사용해야 한다.
...
**(A)** : Inbound Endpoint는 반대 방향으로, 온프레미스 네트워크에서 AWS VPC 내부 리소스의 DNS를 해석할 때 사용한다. 온프레미스 DNS Resolver가 Route 53 Resolver로 쿼리를 보내는 진입점이다.

**(B) 정답** : Outbound Endpoint는 VPC의 Route 53 Resolver에서 온프레미스 DNS Resolver로 DNS 쿼리를 전달하는 역할을 한다. VPC 내에서 온프레미스 리소스 도메인을 해석해야 하므로 DNS 쿼리가 AWS에서 온프레미스 방향으로 나가는 Outbound 구성이 필요하다.
