...
**(A)** : Multi-AZ는 SYNC 복제를 사용하여 재해 복구 목적으로만 사용된다. Standby 인스턴스는 읽기 트래픽을 처리하지 않으므로 읽기 분산에 적합하지 않다.

**(B) 정답** : Read Replica는 비동기(ASYNC) 복제를 통해 읽기 트래픽을 분산하여 메인 DB의 부하를 줄인다. 최대 15개까지 생성할 수 있어 읽기 성능을 크게 향상시킬 수 있다.
...
**(A)** : 일반 RDS for SQL Server는 관리형 서비스이지만 OS에 대한 접근이 불가하다. 기본 OS 커스터마이징 요구사항을 충족하지 못한다.

**(B)** : Aurora는 MySQL과 PostgreSQL만 호환하며 SQL Server(T-SQL)를 지원하지 않는다. MS SQL Server 마이그레이션에 사용할 수 없다.

**(C) 정답** : RDS Custom은 Oracle과 MS SQL Server에 대해 OS 및 DB에 대한 전체 관리자 접근을 제공한다. SSH 또는 SSM Session Manager로 기본 EC2 인스턴스에 접근하여 OS 수준의 커스터마이징이 가능하다.
...
**(B) 정답** : Aurora Database Cloning은 copy-on-write 프로토콜을 사용하여 초기에는 원본 데이터 볼륨을 공유하므로 데이터 복사가 즉시 이루어진다. 변경된 데이터만 추가 스토리지를 사용하여 매우 빠르고 비용 효율적이다.
...
**(B) 정답** : RDS Proxy는 DB 연결을 풀링하여 공유함으로써 동시 연결 수를 대폭 줄이고 DB 리소스 부담을 감소시킨다. Lambda + RDS 조합에서 연결 수 급증 문제를 효과적으로 해결하며 장애 조치 시간도 66% 단축한다.
...
**(A)** : Memcached는 멀티스레드와 샤딩을 지원하지만 데이터 영속성이 없고 Multi-AZ 고가용성을 지원하지 않는다. 단순 키-값 구조만 제공하여 Sorted Sets를 사용할 수 없다.

**(B) 정답** : Redis는 AOF persistence를 통한 데이터 영속성, Multi-AZ Auto-Failover를 통한 고가용성, Sorted Sets를 포함한 복잡한 데이터 구조를 모두 지원한다. 실시간 리더보드 구현에 최적화된 솔루션이다.
...
**(B) 정답** : 올바른 절차는 비암호화 DB 스냅샷 생성 → 암호화된 상태로 스냅샷 복사 → 암호화된 스냅샷에서 새 RDS 인스턴스 복원이다. AWS 공식 권장 방법이다.
...
**(B) 정답** : Aurora Global Database는 1초 미만의 크로스 리전 복제 지연과 1분 미만의 RTO로 재해 복구 승격을 제공한다. 최대 5개 Secondary Region, Region당 최대 16개 Read Replica를 지원하여 두 요구사항을 정확히 충족한다.
...
**(C) 정답** : Session Store 패턴은 세션 데이터를 ElastiCache에 중앙 집중식으로 저장하고 TTL로 세션 만료를 관리한다. 사용자가 다른 EC2 인스턴스로 연결되어도 ElastiCache에서 세션을 조회하여 로그인 상태를 유지할 수 있다.
