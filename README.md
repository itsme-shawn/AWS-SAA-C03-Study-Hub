...
- Ditectrev 덤프 750문제 (배치 15개) + AWS 공식 샘플 문제 포함
...
│   └── ...              # 31개 섹션
├── practice/            # 연습 문제
│   ├── dumps/           # 덤프 문제 세트 (Markdown)
│   │   ├── aws-official-sample.md
│   │   ├── ditectrev-batch-01.md  # Q1-Q50
│   │   └── ...          # batch-02 ~ batch-15 (총 750문제)
│   └── reviews/         # 섹션별 Q&A
└── web/                 # 웹 뷰어 앱
    ├── scripts/
    │   └── generate-data.mjs   # 노트 + 덤프 → JSON 변환 스크립트
    └── src/
```
...
```
notes/**/*.md  +  practice/dumps/*.md
                ↓
generate-data.mjs   ← notes/ 와 dumps/ 디렉토리를 읽어 파싱
                ↓
web/src/data/content.json   ← 섹션 노트 + 퀴즈 + 덤프 데이터 통합
                ↓
Vite + React 앱   ← content.json을 기반으로 UI 렌더링
```

- 각 섹션의 `notes/<section>/README.md` 안에 `## Practice Questions` 섹션이 있으면 자동으로 퀴즈로 파싱됩니다.
- `practice/dumps/*.md` 파일은 덤프 문제 세트로 파싱되어 웹 뷰어에서 별도 뷰로 제공됩니다.
...
| 기능 | 설명 |
|------|------|
| 강의 노트 열람 | 31개 섹션 Markdown 노트 (목차 + 스크롤 점프) |
| 섹션 퀴즈 | 각 섹션 연습 문제 풀기 |
| 덤프 문제 뷰어 | Ditectrev 750문제 + AWS 공식 샘플 배치별 풀기 |
| Mock Exam | 전체 섹션 랜덤 시험 모드 |
| 오답 노트 | 틀린 문제 자동 저장 (섹션 퀴즈 + 덤프 공통) |
| 북마크 | 중요 문제 즐겨찾기 |
| 검색 | 노트/문제 전문 검색 |
| 다크모드 | 라이트/다크 테마 전환 |
